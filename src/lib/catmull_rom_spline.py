...
def catmull_rom_one_point(x, v0, v1, v2, v3):
    ...
    """
    # print "catmull_rom_one_point ", x, v0, v1, v2, v3
    c1 = 1. * v1
    c2 = -.5 * v0 + .5 * v2
    ...
    for i in range(len(p_x) - 1):
        ...
        if i == 0:
            ...
            [v0, v1, v2, v3] = catmull_rom_scale_inputs(
                ...
                p_y[1],
                p_y[2],
                p_x[0] - (p_x[1] - p_x[0]),  # estimated start point,
                p_x[0],
                p_x[1],
                p_x[2])
            y_intpol[:res] = np.array([catmull_rom_one_point(x, v0, v1, v2, v3) for x in
                                       np.linspace(0., 1., res, endpoint=False)])
            ...
            y_intpol[i * res:-1] = np.array([catmull_rom_one_point(x, v0, v1, v2, v3) for x in
                                             np.linspace(0., 1., res, endpoint=False)])
        else:
            [v0, v1, v2, v3] = catmull_rom_scale_inputs(
                p_y[i - 1],
            ...
            y_intpol[i * res:(i + 1) * res] = np.array(
                [catmull_rom_one_point(x, v0, v1, v2, v3) for x in
                 np.linspace(0., 1., res, endpoint=False)])
